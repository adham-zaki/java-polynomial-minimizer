...
public class Pro3_zakiadha {
	...
	private static Polynomial readPolynomial() throws Exception {
		...
		// An error you should throw if it occurs
		//   throw new Exception("ERROR: " + filename + " is empty");
	...
	public static void getParamsUser(Minimizer m) {
		...
		System.out.println();
		double eps   = Pro3_zakiadha.getDouble("Enter tolerance epsilon [0.000001,infinity]: ", 0.000001, Double.MAX_VALUE);
		m.setEps(eps);

		int max_iter = Pro3_zakiadha.getInteger("Enter maximum number of iterations [1,1000]: ", 1, 1000);
		m.setMaxIter(max_iter);

		double alpha = Pro3_zakiadha.getDouble("Enter step size alpha [0.000001,1]: ", 0.000001, 1.0);		
		m.setStepSize(alpha);
		...
		while (true) {
			...
			double val = Pro3_zakiadha.getDouble("Enter starting value for variable " + var + ": ", -Double.MAX_VALUE, Double.MAX_VALUE);	
			x0.put(var, val);
		}
		m.setX0(x0);
	...
	public static void printResults(Minimizer m) {
		...
		System.out.println("Last gradient norm:   " + String.format("%6.4f", m.getLastGradNorm()));
		System.out.println("Point at termination: " + VectorUtils.vectorToString(m.getLastPoint()));
